...
from app1.models import * # type: ignore
import re
import json
from .models import driver_info, garbageDetails, schedule, user_details
from .models import DriverDetails, PickupSchedule 
from rest_framework.views import APIView # type: ignore
from rest_framework.views import Response # type: ignore
from rest_framework.views import status # type: ignore
from app1.serializers import * # type: ignore
...
import random
import shutil
# from fontawesome.fields import IconField # type: ignore
from random import randint
import smtplib
from django.core.files.base import ContentFile
from .models import garbageDetails
...
from .serializers import driverScheduleSerializer, garbageInfoSchedule1, user_infoSerializer
...
def home(request):
    diction={}
    diction['value']=0
    return render(request ,"home.html",{'diction':diction})

def signup_user(request):
    diction={}
    diction['value']=0
    return render(request ,"signup_user.html", {'diction':diction})

def login_user(request):
    diction={}
    diction['value']=0
    return render(request ,"login_user.html", {'diction':diction})

def signup_driver(request):
    diction={}
    diction['value']=0
    return render(request ,"signup_driver.html", {'diction':diction})

def login_admin(request):
    diction={}
    diction['value']=0
    return render(request ,"login_admin.html", {'diction':diction})
...
#         if user is not None:
#             # This is the crucial step for an ADMIN login.
#             # We check if the user is a staff member.
#             if user.is_staff:
#                 login(request, user)
#                 # Redirect to your existing 'adminPage' URL
#                 return redirect('adminPage')
#             else:
#                 # If they are a regular user, deny access.
#                 messages.error(request, 'You do not have permission to access the admin area.')
#                 return render(request, 'adlog.html') # Or your admin login template name
#         else:
...
def adminHome(request):
    # Ensure the user is a staff member before allowing access
    if not request.user.is_staff:
        # You can redirect non-admins to a different page if you want
        return redirect('home') 

    # 1. Count regular users (excluding staff/admins)
    user_count = User.objects.filter(is_staff=False).count()

    # 2. Count all registered drivers
    driver_count = DriverDetails.objects.count()

    # 3. Count all scheduled pickups
    schedule_count = PickupSchedule.objects.count()

    # 4. Prepare the context dictionary for the template
    context = {
        'diction': {
    ...
    # 5. Render the adminHome.html template with the data
    return render(request, 'adminHome.html', context)

def check_schedule(request):
    # This assumes your custom user details model has a 'user' field
    # linking it to Django's built-in User model.
    user_details_obj = user_details.objects.get(user=request.user)
    ...
    if not days:
        ...
        return redirect('create_schedule') # Redirect back to the form

    # Convert day numbers to a single integer value
    day_value = sum(int(day) for day in days)
    
    # Check if a schedule already exists for this user to prevent duplicates
    if schedule.objects.filter(user=user_details_obj).exists():
        messages.warning(request, "A schedule already exists for you. Please update it instead.")
        return redirect('view_schedule') # Redirect to a page where they can view/update
    
    # Create the new schedule object
    new_schedule = schedule(
        days=day_value,
        user=user_details_obj,
        address=user_details_obj.userAddress,
        specialInst=request.POST.get('specialinst', ''),
        community=bool(community), # Converts list to True if not empty, False if empty
        noofHouses=0
    )

    if addch:
        # If user wants to change address, redirect them to a different page
        # This is a better flow than trying to handle it within the same view
        new_schedule.save() # Save the partial schedule first
        return redirect('address_change_view') # Assumes you have a URL named 'address_change_view'

    else:
        new_schedule.save()
        messages.success(request, "Your pickup has been scheduled successfully!")
        return redirect('view_schedule') # Redirect to the success/view page
...
def see_schedules(request):
    ...
    # Build a dictionary similar to your previous code
    diction = {
        "result": "true" if user_schedules.exists() else "false",
        "items": [(i, s) for i, s in enumerate(user_schedules, start=1)],
        "value": 0,  # You can set this to 1 if you want to trigger alerts
        "done": "Schedule loaded successfully"
    }

    return render(request, "see_schedules.html", {"diction": diction})

def check_schedules(request):
    return redirect('see_garbage')
...
def see_all_users(request):
    return render(request ,"see_all_users.html",)
...
def user_login_Check(request):
    ...
    if a:
        ...
        if flag==1:
            diction['result']="true"
            diction['sidenav']=1
            return render(request ,"logged_In_user.html",{'diction':diction})
        else:
...
def create_schedule_view(request):
    # This view will show the initial schedule creation form.
    # We set pagevalue to 0 to match the first condition in your template.
    context = {
        'diction': {
            'pagevalue': 0,
            # You might need to pass the user's address here as well
...
def create_schedule(request):
    if request.method == "POST":
        ...
        return redirect('see_schedules')  # redirect to the see_schedules page
...
def user_login_Check(request):
    ...
    uname = request.POST.get('uname')   # matches your form field name
    pwrd = request.POST.get('pwrd')

    try:
        user = user_details.objects.get(userUname=uname, userPwrd=pwrd)
        # set session so other views can check login
        request.session['id'] = user.id
        request.session['userUname'] = user.userUname

        # Render the sidebar/dashboard template right away.
        # URL will remain /user_login_Check
        return render(request, "user_sidenav.html", {'diction': user})
...
def driver_login_Check(request):
    if request.method == "POST":
        ...
        try:
            ...
            # go to dashboard/sidebar after login
            return render(request, "sidenav.html", {'diction': user})

        except user_details.DoesNotExist:
            messages.error(request, "Invalid username or password.")
            return render(request, "login_driver.html")  # show login form again

    return render(request, "login_driver.html")

def user_sidenav(request):
    return render(request, "user_sidenav.html")
...
def see_user_details(request):
    ...
    if a:
        ...
        if(flag==1):
            diction=a[i]
    return render(request, "see_user_details.html",{'diction':diction})
...
def update_details(request):
    ...
    if a:
        ...
        if(flag==1):
            diction=a[i]
    print("hi")
    return render(request, "update_details.html",{'diction':diction})
...
def forgot_password(request):
    ...
    diction['pagevalue']= 0
    diction['value'] = 0
    return render(request, "forgotpassword.html",{'diction':diction})


def forgot_password_check(request):
    diction = {}

    # 🟢 Step 0 → Default page (GET request)
    ...
    # 🟢 Step 1 → Email + last 4 digits form
    elif request.method == "POST" and 'email' in request.POST and 'digits' in request.POST:
        email = request.POST.get('email', '').strip()
        digits = request.POST.get('digits', '').strip()

        # 1️⃣ Validate email
        if not email:
            diction['result'] = "Please enter your email."
            diction['pagevalue'] = 0
            diction['value'] = 1
            return render(request, "forgotpassword.html", {'diction': diction})
        ...
        try:
            user = user_details.objects.get(userEmail=email)
            phno = user.userPhno % 10000  # last 4 digits of phone
        except user_details.DoesNotExist:
            diction['result'] = "Email not registered."
            diction['pagevalue'] = 0
            diction['value'] = 1
            return render(request, "forgotpassword.html", {'diction': diction})
        ...
        if len(digits) != 4 or not digits.isdigit():
            diction['result'] = "Please enter exactly 4 digits."
            diction['pagevalue'] = 0
            diction['value'] = 1
            return render(request, "forgotpassword.html", {'diction': diction})

        if digits != str(phno):
            diction['result'] = "Digits do not match the registered phone number."
            diction['pagevalue'] = 0
            diction['value'] = 1
            return render(request, "forgotpassword.html", {'diction': diction})

        # 4️⃣ If valid → Send OTP
        ...
        diction['pagevalue'] = 2
        diction['value'] = 0
        diction['result'] = "OTP sent to your email."
        return render(request, "forgotpassword.html", {'diction': diction})
    ...
    # 🟢 Step 2 → OTP verification
        ...
        if entered_otp == session_otp:
            diction['pagevalue'] = 1
            diction['value'] = 0
            diction['result'] = "OTP verified. Please enter a new password."
            return render(request, "forgotpassword.html", {'diction': diction})
        else:
            diction['pagevalue'] = 2
            diction['value'] = 1
            diction['result'] = "Invalid OTP. Please try again."
            return render(request, "forgotpassword.html", {'diction': diction})

    # 🟢 Step 3 → Password reset
    elif request.method == "POST" and 'pwrd1' in request.POST and 'pwrd2' in request.POST:
        pwrd1 = request.POST.get('pwrd1', '').strip()
        pwrd2 = request.POST.get('pwrd2', '').strip()

        if pwrd1 != pwrd2:
            diction['pagevalue'] = 1
            diction['value'] = 1
            diction['result'] = "Passwords do not match."
            return render(request, "forgotpassword.html", {'diction': diction})
        ...
        if not user_id:
            diction['pagevalue'] = 0
            diction['value'] = 1
            diction['result'] = "Session expired. Please start again."
            return render(request, "forgotpassword.html", {'diction': diction})

        # ✅ Update password in DB (example: storing plain text for now — use hashing in real app)
        user = user_details.objects.get(id=user_id)
        user.userPassword = pwrd1
        user.save()

        diction['pagevalue'] = 0
        diction['value'] = 0
        diction['result'] = "Password reset successful. You can now log in."
...
# def forgot_password_check(request):
#     if request.method == "POST":
#         # Only access POST data here
#         email = request.POST.get('email', None)  # safer
#         if email:
...
def mail(toadd, otp): 
    fromadd="priyashinde2345@gmail.com"
    toadd=toadd
    password="20199000"
    msg=MIMEMultipart()
    msg['From']=fromadd
...
def create_schedule(request):
    ...
    diction['value'] = 0
    diction['pagevalue'] = 0
    return render(request, "create_schedule.html",{'diction':diction})
...
def check_schedule(request):
    ...
    if( community != [] ):
        ...
        return render(request, "view_schedule.html",{'diction':diction})
...
def update_schedule(request):
    ...
    if a:
        ...
        if(flag == 1):
            ...
            diction['value'] = 0
            diction['pagevalue'] = 0
            return render(request, "update_schedule.html",{'diction':diction})
...
def change_schedule(request):
    ...
    t.user_approval = 0
    t.save()
    return render(request, "view_schedule",{'diction':diction})
...
def approved_driver(request):
    ...
    return render(request, "see_all_users_right_side.html", {'diction':diction})
...
def adminHome(request):
    ...
    return render(request , "adminHome.html",{'diction':diction})
    ...
    if request.method == "POST":
        ...
        if data:
            if values['userProfilePicture'] != None:
                ...
                newpath = r"C:\Karmanya\MCA\SYMCA\MINI proj\garbage_management\static\img"+filename
                shutil.move("imageToSave.jpg",r"C:\Karmanya\MCA\SYMCA\MINI proj\garbage_management\static\img")
                os.rename(r"C:\Karmanya\MCA\SYMCA\MINI proj\garbage_management\static\img", newpath)
    ...
    if request.method == "POST":
        ...
        if data:
            ...
            if values['driverLicPicture'] != None:
                ...
                newpath1 = r"C:\Karmanya\MCA\SYMCA\MINI proj\garbage_management\static\img"+filename1
                shutil.move("imageToSave.jpg",r"C:\Karmanya\MCA\SYMCA\MINI proj\garbage_management\static\img")
                os.rename(r"C:\Karmanya\MCA\SYMCA\MINI proj\garbage_management\static\img", newpath1)
...
#             i+=1
#     print(diction)
#     return render(request, "see_garbage.html", {'diction':diction})
...
def see_garbage(request):
    ...
    # The dummy data logic can stay if you want.
    if garbageDetails.objects.count() == 0:
        # ... (your code to create dummy g1 and g2) ...

        garbage_items = garbageDetails.objects.all()
    context = {
        "garbage_items": garbage_items
    }
    return render(request, "add_garbage.html", context)

# app1/views.py

def add_garbage(request):
    # This block processes the submitted form data
    if request.method == 'POST':
        new_garbage = garbageDetails(
            user_id=request.user.id,
            geotag=request.POST.get('geotag'),
            date=request.POST.get('date'),
            time=request.POST.get('time'),
            garbagePicture=request.FILES.get('garbagePicture')
        )
        new_garbage.save()
        # After saving, redirect to the page that shows the list
        return redirect('add_garbage')
    
    # This block just shows the blank form page
    return render(request, 'add_garbage.html')
